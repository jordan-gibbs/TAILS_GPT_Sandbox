...
def setup_assistant(client, assistant_name):
    ...
    assistant = client.beta.assistants.create(
        name=assistant_name,
        instructions=f"""
            You are an intelligent and helpful research assistant. Your name is {assistant_name}. You will work with 
            the user to help them learn new updates on AI advancements from data within a json file. You will analyze 
            the json file, find the most relevant papers to the user's learning request, and output a summary of the 
            articles their importance, and the links to the papers in one message. Always output the links to the 
            papers after your summarize them. 
            """,
...
def main_loop():
    ...
    user_choice = input("Type 'n' to make a new agent. Press 'Enter' to choose an existing "
                      "session. ")
    if user_choice == 'n':
        scrape_ai(start_date='2023-12-01', end_date='2023-12-01')
